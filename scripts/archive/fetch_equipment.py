#!/usr/bin/env python3
"""
Fetch equipment data for Epsom 2026-06-05 and 2026-06-06.
...
def fetch_rp_racecard(race_date: str) -> dict[str, dict]:
    """
    Fetch Racing Post Epsom racecard page, extract equipment from __NEXT_DATA__.
    Returns {horse_name: {equipment: [codes], first_time_use: [codes], wind_surgery: bool|None}}.
    """
    url = f"https://www.racingpost.com/racecards/{RP_EPSOM_ID}/epsom/{race_date}"
...
def main():
    ...
    with open(MARKET_BASELINE) as f:
        baseline = json.load(f)
    horses = baseline.get("horses", {})
    print(f"Loaded {len(horses)} horses from market-baseline.json", file=sys.stderr)
    ...
    for i, race_date in enumerate(race_dates):
        ...
        rp = fetch_rp_racecard(race_date)
        all_rp.update(rp)
        url = f"https://www.racingpost.com/racecards/{RP_EPSOM_ID}/epsom/{race_date}"
        source_url_map[race_date] = url
        if rp:
            sources_used.append(f"racingpost.com/racecards/epsom/{race_date}")
