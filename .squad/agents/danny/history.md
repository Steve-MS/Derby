...
- 2026-06-08 (Danny-6 publish audit): v0.4 #6 is runtime/RUNBOOK ready but not tag-ready as a consumer skill. The blockers are publish-facing, not model-facing: root README still frames the repo as Epsom-only, `.squad/skills` still contains several Epsom-shaped workflows, and release metadata is unstamped. Bias-to-ship verdict: ALMOST; fix README + skill templates + tag metadata before `v0.4.0`.
