...
- When source import failures need to become explicit operator blockers
...
2. **Apply artifact-specific freshness windows.** Raw declarations from `fetch --from-file` can be older than live-runner checks; live identity and bookmaker enrichments need tighter windows.
3. **Check dependency ordering.** Browser save precedes `fetch --from-file`; raw racecards precede enrichment; enrichment precedes scores; scores precede reports; bets precede rendered cards; slips mirror bets.
4. **Promote suspicious small files to hard failures.** Known SPA-shell, auth-failure, wrong-page, or parser-diagnostic outputs should be size/schema-gated, not merely parsed.
...
- Treat deprecated `scripts/scrape_*.deprecated.py` files as historical references only.
